...
def load_image(path,filename, do_resample=True, do_normalize=True):
    ...
    if path == cfg.path:
        data_type = "fixed"
    elif path == cfg.path_seg:
        data_type = "label"

    if do_normalize:
        data = normalize(data, data_type)
...
def load_template(filenames):
    images = []
    for file_name in filenames:
        image = load_image(cfg.path, file_name, do_resample=False, do_normalize=False)
        images.append(image)
...
def normalize(img, eps=np.finfo(np.float).min, data_type="fixed"):
    ...
    if data_type == "label":
        ...
        cfg.norm_min_v = cfg.norm_min_v_img
        cfg.norm_max_v = cfg.norm_max_v_img
        ...
        percentile_05 = np.percentile(img, 15)
...
def getdatalist_from_csv(csv):
    """
        Reads a csv file containing the filenames of the images, and returns the lists of filenames respectively.

        Args:
            csv (str): path to the csv file containing the filenames of the imagess

        Returns:
            list: filenames of the images
        """
    # Read the csv files into dataframes
    data_list = pd.read_csv(csv, dtype=object,sep=';').values
